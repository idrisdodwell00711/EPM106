...
for idx, img in enumerate(x_train):
      binary = [1.0 if n > (127/255) else 0.0 for n in img ]
      x_train[idx] = binary
...
class SVM:
    ...
    def __init__(self, learning_rate=0.001, lambda_param=0.001, n_iters=5):
...
for digit in digits:
    ...
    model = SVM(learning_rate=0.001, lambda_param=0.001, n_iters=16)
...
def predict_multiclass(X):
    ...
    for digit, model in ovr_models.items():
...
for i, idx in enumerate(selected_indices):
    ...
    plt.imshow(x_test[idx].reshape(14, 14), cmap='gray')
